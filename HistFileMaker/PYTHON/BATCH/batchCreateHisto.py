...
class BatchSubmission(object):
    ...
    def createBatchSub(self, bMainCfg, cmdLocation, trunkLocation, count):
        ...
        f.write('python PYTHON/MODULES/createHistFile.py --MainConfigFile=' + bMainCfg + ';')
        f.close()
    ...
    def submission(self, l_bSub, queue, email):
        ...
        for i in xrange(len(l_bSub)):
            subCmd = 'qsub -N histo_' + str(i) + ' -q ' + queue + ' -M ' + email + ' -S /bin/bash ' + str(l_bSub[i])
            print subCmd
            os.system(subCmd)
            time.sleep(0.01)
